...
struct sanitationCompleted: Decodable, Identifiable {
    let id = UUID()
    let srNumber: String
    let srType: String
    let status: String
...
class SanitationComplete: ObservableObject {
...
struct SanitationCompleted: View { // make sure that this is differnet
    @StateObject var viewModel = SanitationComplete()
