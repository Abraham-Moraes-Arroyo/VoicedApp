...
struct SanitationOpen: Decodable, Identifiable {
    let id = UUID()
    let srNumber: String
    let srType: String
    let status: String
...
class SanitationsInComplted: ObservableObject {
...
struct SantinationOpen: View { // make sure that this is differnet
    @StateObject var viewModel = SanitationsInComplted()
